...
export function BookingDialog({ phone }: { phone: string }) {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      name: draft.name,
      phone: draft.phone,
      appliance: (appliance as ApplianceType | undefined) ?? (draft.appliance || "ac"),
  ...
  const onSubmit = form.handleSubmit(async (values) => {
    try {
      ...
      resetDraft();
      form.reset({ name: "", phone: "", appliance: "ac", address: "", message: "" });
      setOpen(false);
    } catch (err) {
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Tell us about the appliance and we&apos;ll call back to confirm a same-day visit.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={onSubmit} noValidate>
          <FieldGroup>
              ...
              <Controller
                ...
                render={({ field, fieldState }) => (
                  <Field data-invalid={fieldState.invalid}>
                    ...
                    <Input id="bk-name" placeholder="Full name" autoComplete="name" {...field} />
            ...
            <Controller
              ...
              render={({ field, fieldState }) => (
                <Field data-invalid={fieldState.invalid}>
                  ...
                  <Input id="bk-address" placeholder="e.g. Salt Lake, Sector V" {...field} />
                  <FieldError errors={fieldState.error ? [fieldState.error] : undefined} />
                </Field>
              )}
            />
            <Controller
              ...
              render={({ field, fieldState }) => (
                <Field data-invalid={fieldState.invalid}>
                  <FieldLabel htmlFor="bk-message">Problem description (optional)</FieldLabel>
                  <Textarea
                    id="bk-message"
                    rows={3}
